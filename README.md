...
当前状态：阶段 2 已完成登录、身份适配和 RBAC 最小闭环；阶段 3 已完成旧种子源导入、共享数据源池、默认工作台源链接、工作台级标签配置、adapter 框架和手动 RSS 抓取到 `raw_items` 的最小链路；前端工作台导航已改为数据库驱动，数据源页已能展示并编辑当前工作台的源启用、权重、日限和一级/二级标签路径。下一步补抓取调度，然后进入阶段 4 的标准化与去重。
...
- 数据源页每个源都有“配置”入口，可设置当前工作台启用状态、权重、日限、标签集和一级/二级标签路径。
