...
class AcceptanceConfig:
    ...
    workspace_prefix: str = "hardware_intel"
...
class AcceptanceResult:
    ...
    daily_tech_md_path: Path
    daily_tech_html_path: Path
    daily_custom_md_path: Path
    daily_custom_html_path: Path
    weekly_md_path: Path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the InfoWatchtower blueprint §9 acceptance flow.")
    ...
    parser.add_argument("--workspace-prefix", default=os.environ.get("ACCEPTANCE_WORKSPACE_PREFIX", "hardware_intel"))
    parser.add_argument(
        "--rss-host",
    ...
    parser.add_argument("--week-key", default=os.environ.get("ACCEPTANCE_WEEK_KEY", iso_week_key(date.today())))
...
def run_acceptance(client: HttpClient, config: AcceptanceConfig) -> AcceptanceResult:
    ...
    bootstrap_workspace_code = _unique_code("acceptance_boot", suffix)
    workspace_code = _unique_code(config.workspace_prefix, suffix)
    invite_username = f"{config.invite_username}-{suffix[-6:]}"
    report_format_code = f"weekly_exec_{suffix[-6:]}"
    ...
    with local_rss_server(
        ...
        _record_json(evidence_dir / "01_login_admin.json", _login(client, config.admin_username, config.admin_password))

        bootstrap_workspace = expect_json(
            _post(
                client,
                "/api/workspaces",
                json={
                    "code": bootstrap_workspace_code,
                    "name": f"验收邀请工作台 {suffix[-6:]}",
                    "description": "蓝图 §9 邀请流程的临时工作台。",
                    "default_domain_code": "ai",
                },
            ),
        )
        _record_json(evidence_dir / "02_bootstrap_workspace.json", bootstrap_workspace)
        ...
        invite = expect_json(
            _post(
                ...
                json={
                    ...
                    "workspaces": [{"code": bootstrap_workspace_code, "workspace_role": "owner"}],
                    "expires_in_days": 7,
                },
            ),
        )
        _record_json(evidence_dir / "03_invite.json", invite)
        ...
        try:
            ...
            _record_json(evidence_dir / "04_invite_accept.json", accepted)
            ...
            workspace = expect_json(
                _post(
                    ...
                    json={
                        "code": workspace_code,
                        "name": f"硬件情报验收工作台 {suffix[-6:]}",
                        "description": "蓝图 §9 自动验收工作台。",
                        "default_domain_code": "ai",
                    },
                ),
            )
            _record_json(evidence_dir / "05_workspace.json", workspace)
            ...
            shared_source = expect_json(
                _post(
                    ...
                    "/api/sources",
                    json={
                        "workspace_code": bootstrap_workspace_code,
                        "name": f"验收共享 RSS {suffix[-6:]}",
                        "source_type": "rss",
            ...
            published_daily = expect_json(_post(invited_client, f"/api/daily-reports/{daily_report_id}/publish", json={}))
            ...
            published_weekly = expect_json(_post(invited_client, f"/api/weekly-reports/{weekly_report_id}/publish", json={}))
            _record_json(evidence_dir / "13_weekly_publish.json", published_weekly)

            daily_tech_md_path = _write_response(
                evidence_dir / "daily_tech_insight.md",
                _get(invited_client, f"/api/daily-reports/{daily_report_id}/renditions/tech_insight_v1/export?target=md"),
            )
            daily_tech_html_path = _write_response(
                evidence_dir / "daily_tech_insight.html",
                _get(invited_client, f"/api/daily-reports/{daily_report_id}/renditions/tech_insight_v1/export?target=html"),
            )
            daily_custom_md_path = _write_response(
                evidence_dir / "daily_custom_format.md",
                _get(invited_client, f"/api/daily-reports/{daily_report_id}/renditions/{report_format_code}/export?target=md"),
            )
            daily_custom_html_path = _write_response(
                evidence_dir / "daily_custom_format.html",
                _get(invited_client, f"/api/daily-reports/{daily_report_id}/renditions/{report_format_code}/export?target=html"),
            )
            weekly_md_path = _write_response(
                evidence_dir / "weekly_report.md",
                _get(invited_client, f"/api/weekly-reports/{weekly_report_id}/renditions/{report_format_code}/export?target=md"),
            )
            weekly_html_path = _write_response(
                evidence_dir / "weekly_report.html",
                _get(invited_client, f"/api/weekly-reports/{weekly_report_id}/renditions/{report_format_code}/export?target=html"),
            )

            company_sql = expect_json(_post(invited_client, f"/api/exports/company-sql/daily-reports/{daily_report_id}", json={}))
    ...
    return AcceptanceResult(
        bootstrap_workspace_code=bootstrap_workspace_code,
        workspace_code=workspace_code,
        ...
        daily_tech_md_path=daily_tech_md_path,
        daily_tech_html_path=daily_tech_html_path,
        daily_custom_md_path=daily_custom_md_path,
        daily_custom_html_path=daily_custom_html_path,
...
def _login(client: HttpClient, username: str, password: str) -> dict[str, Any]:
    return expect_json(_post(client, "/api/auth/login", json={"username": username, "password": password}))
...
def materialize_daily_report_for_sql(daily_report_id: str) -> dict[str, Any]:
    ...
    with session_factory() as session:
        report = session.scalar(select(DailyReport).where(DailyReport.id == daily_report_id))
        if report is None:
            raise AcceptanceError(f"daily report not found for materialization: {daily_report_id}")
        items = session.scalars(select(DailyReportItem).where(DailyReportItem.daily_report_id == daily_report_id)).all()
        for index, item in enumerate(items, start=1):
            ...
            for field_name in SQL_CONTENT_FIELDS:
                content[field_name] = str(content.get(field_name) or _fallback_content(field_name, news.title))
...
def _fallback_content(field_name: str, title: str) -> str:
    labels = {
...
def run_backup(evidence_dir: Path) -> Path | None:
    ...
    env = {**os.environ, "BACKUP_DIR": str(backup_dir), "BACKUP_KEEP": "3"}
    result = subprocess.run(
...
def local_rss_server(bind_host: str, public_host: str, port: int, day_key: str):
    ...
    try:
        ...
        yield RssServerHandle(public_host=public_host, port=actual_port)
    finally:
        server.shutdown()
        server.server_close()
...
def _unique_code(prefix: str, suffix: str) -> str:
...
def result_summary(result: AcceptanceResult) -> dict[str, Any]:
    return {
        "bootstrap_workspace_code": result.bootstrap_workspace_code,
        ...
        "daily_tech_md_path": str(result.daily_tech_md_path),
        "daily_tech_html_path": str(result.daily_tech_html_path),
        "daily_custom_md_path": str(result.daily_custom_md_path),
        "daily_custom_html_path": str(result.daily_custom_html_path),
