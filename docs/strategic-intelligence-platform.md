...
当前 `config/taxonomy/news_categories.json` 里的 10 个标签是历史 AI 日报和公司 SQL 导出的兼容标签，不是系统的长期分类上限。

长期分类分两层：

```text
domain / board          业务板块：AI、硬件、半导体、政策、竞品等
taxonomy / labels       板块内标签：模型、芯片、供应链、终端、监管、渠道等
```
...
这样当前 AI SQL 仍然兼容旧平台，后续硬件板块可以新增自己的 domain pack，不需要改主流程。
