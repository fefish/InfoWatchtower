...
当前仓库已完成阶段 0-2，并完成阶段 3 的旧种子源导入、共享数据源池、默认工作台源链接、工作台统一标签策略、adapter 框架和手动 RSS 抓取到 `raw_items` 的最小链路。前端工作台导航已改为从 `workspaces/workspace_sections` 读取，数据源页采用信息流式共享源列表，右侧展示工作台统一一级/二级标签策略，并编辑当前工作台对单个源的启用状态、权重和日限。`planning_intel` 和 `ai_tools` 的默认标签策略已在后端隔离。下一步补抓取调度，然后进入阶段 4 的标准化与去重。
