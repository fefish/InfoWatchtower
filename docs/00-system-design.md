...
2. P0 公网/内网部署与登录安全：当前已有生产 Compose、Caddy 反向代理、生产 env 模板、部署检查脚本、`install.sh`/`upgrade.sh`、启动自动迁移、首次运行 Setup、备份/恢复脚本、登录限流、管理员邀请、改密/忘记密码/管理员代重置、会话密钥启动自检、生产 `DATABASE_URL` 自检、改密后旧 cookie 失效、OIDC adapter Protocol 预留，以及主要工作台业务 API 的 membership gate；下一步补公司 IDaaS code flow adapter 预留，并在 WP2 后执行干净环境 §9 全量业务验收证据留存。
