...
1. P0 SQL 导出增强：当前已支持选择已发布日报、查看导出历史、预览和下载 SQL；下一步补导出前字段长度、URL 长度、HTML 污染校验，以及从 SQL 条目追溯到 daily item、generated news、news item、raw item 和 source。
2. P0 公网/内网部署与登录安全：补生产 Compose、反向代理、备份恢复、登录限流、默认密码治理、Google OIDC 预留和公司 IDaaS code flow adapter 预留。
3. P0 同步包实质能力：当前 `/sync` 已能记录同步运行，后续补真实同步包导出、下载、导入、幂等写入、冲突检查和同步审计；继续坚持公网公开信号向内网同步，内网用户反馈默认不回流公网。
