...
当前仓库已完成阶段 0-2：可本地 Docker Compose 启动，已具备 FastAPI/Vue/PostgreSQL/Redis 骨架、数据库追溯链路、登录、身份适配和 RBAC 最小闭环。下一步进入数据源导入、adapter 框架和 RSS 抓取。
...
```text
workspace_code          所属工作台，如 planning_intel/ai_tools
...
section_key/module_key  选择页面或可选附加模块
...
- `planning_intel` 和 `ai_tools` 都默认拥有数据源、候选池、日报、周报、专题、SQL 导出。
- `ai_tools` 可以额外叠加工具目录和工具任务，但不能拆掉主情报链路。
- 两个工作台可以复用同一个 RSS、wiseflow 或论文源；复用关系写在 `workspace_source_links`。
- `domain_code=ai` 和 `domain_code=hardware` 是内容板块，不是工作台。

规则：

- 不要把 `domain_code` 当成 UI 工作台边界。
- 不要为了 AI 工具桌面另起一个前后端仓库。
