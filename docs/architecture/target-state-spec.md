...
> **状态（2026-07）：WP1-WP3 已全部完结**，本文保留为历史实现规格附录。四种部署
> 形态（standalone/cloud/intranet/extranet）的目标态与后续工作以
> `docs/deployment/deployment-topology.md` 为唯一验收基准；细节与契约冲突时以
> `config/contracts/*.json`（如 `auth_modes.json`）为准。
...
用户视角（目标态）：
  ...
  → 同一套代码按 DEPLOY_MODE 支撑四种部署形态（deployment-topology.md）：
     standalone 本地一键 Docker 自用
     cloud      云主机官方站（邀请制、viewer 只读）
     extranet   公网 OIDC SSO 采集发布，开放 GET /api/sync/feed
     intranet   内网门户 iframe 嵌入，header 登录复用外层身份，禁采集、pull-only
  → 登录身份可迁移：local/public_password/oidc/intranet_header 共用同一
     users/RBAC/session 模型，extranet→intranet 的成品经 sync feed 单向流动

现状标注：流水线/成稿/建台/自建源 ✅ 已实现；
账户生命周期 ✅ WP1；一键部署与 Setup ✅ WP3；建台向导与成员管理 ✅ WP2；
部署形态与联动同步 ✅ WP4-WP7（见 deployment-topology.md）；
§8/§9 全量验收入口已脚本化为 `scripts/run_full_acceptance.py`。
```
...
- 已有：`POST /api/auth/login`（public_password 模式，签名 cookie 会话）、
  ...
  管理员邀请、邀请接受建号、改密、忘记密码恒定响应、管理员代重置、登录限流、
  会话 max-age/secure cookie 配置、改密后旧 cookie 失效、通用 OIDC code flow + PKCE。
- 用户来源：启动时 env `AUTH_BOOTSTRAP_ADMIN_*` 可创建首个管理员；已有用户时 seed 不覆盖；
  后续普通用户通过管理员邀请进入。
- 已有：带 `workspace_code` 的主要业务 API 执行 `workspace_memberships` 权限检查，
  `super_admin` 可绕过，viewer/member/admin/owner 按工作台内角色控制。
- 已有：首次运行 Setup 向导（空 users 表时创建首个 super_admin，已有用户后 410）。
- 已有：工作台成员管理 API/UI，owner/admin 或 super_admin 可增删成员。
- **已补**：通用 `AUTH_MODE=oidc`，具体企业 provider 只需配置 env 和 claims 字段映射。
...
- 已有：`POST /api/workspaces` 建台（自动配齐核心分区/标签策略/内置成稿格式/owner
  成员）、AppShell 三步建台向导、`PATCH /api/workspaces/{code}` 工作台更新/停用、
  `GET/POST/DELETE /api/workspaces/{code}/members` 成员管理、`POST /api/sources`
  自建源入共享池、标签策略 API、成稿格式注册表、分组导航
  （`workspace_sections.config_json.group`）、`hardware` domain pack 样例和
  `docs/backend/extension-recipes.md`。

部署现状（`deploy/`）：

- 已有：`docker-compose.local.yml`、`docker-compose.prod.yml`（pg/redis/api/worker/
  scheduler/caddy）、`env.production.example`、部署检查脚本。
- 已有：`deploy/install.sh`/`deploy/upgrade.sh`、API 容器启动自动迁移、生产 env 检查、
  备份/恢复脚本、首次运行 Setup 向导、`AUTH_MODE=public_password` 缺
  `AUTH_SESSION_SECRET` 和生产缺 `DATABASE_URL` 的启动自检。
- 已有：干净 Docker 环境可复用 `scripts/run_full_acceptance.py` 执行最终 §9 全量业务验收，
  输出目录为 `outputs/acceptance/<timestamp>/`；当前干净 Docker 证据为
...
- 密码规则：最小 8 位（与 `config/contracts/auth_modes.json` 的
  client_validation 和 `backend/app/schemas/auth.py` `min_length=8` 对齐），
  禁止与 username 相同；哈希沿用现有 `app/auth/passwords.py`（bcrypt）。
...
- cookie 增加过期（实现态 env 为 `AUTH_SESSION_TTL_SECONDS`，默认 43200 即
  12 小时，见 `auth_modes.json` session_hardening 与
  `backend/app/core/config.py`）；签名 payload 加入
  `password_changed_at` 类似 nonce（可用 user.updated_at）实现改密后旧会话失效。

OIDC 已实现通用 provider flow：`GET /api/auth/oidc/start` 生成 state/nonce/PKCE 并跳 IdP，
`GET /api/auth/oidc/callback` 交换 token、读取 userinfo、映射本地用户并签发现有 session。
...
4. **domain pack 样例 `hardware`**：新增
   ...
   为验收核心。
5. **扩展配方文档** `docs/backend/extension-recipes.md`：R1 新建工作台（界面/API）、
...
5. **备份/恢复**：`scripts/backup_db.sh`（pg_dump 到时间戳文件 + 保留 N 份）、
   ...
   `docs/deployment/deployment-ops.md` 演练步骤。
...
2. `raw_items.raw_payload_json` 只被同源同 entry_key 重抓刷新为最新抓取快照，
   编辑/流水线等下游环节永不回写；去重在 news 之后推荐之前；
...
6. 前端视觉基线 Liquid Glass（`docs/implementation/api-and-ui-implementation.md` §3.1）；
...
`config/contracts/auth_modes.json`、`docs/deployment/auth-unified-login.md`、
`docs/deployment/deployment-ops.md`）。

## 8. 端到端完成定义（全局验收）

在干净 Docker 环境执行一次完整走查并留下记录（截图或脚本输出）。推荐使用脚本化入口：
...
脚本会驱动 Setup/登录、邀请 `editor_admin` 用户、建临时邀请工作台、由受邀用户新建
`hardware_intel_*` 工作台、启用共享 RSS 源和自建 RSS 源、自定义标签策略、注册自定义
周报格式、跑日报流水线、导出日报双版 MD/HTML、生成周报 MD/HTML、导出公司 SQL 并运行
`scripts/validate_company_sql.py`；PostgreSQL 环境下还会调用 `scripts/backup_db.sh` 留备份证据。
