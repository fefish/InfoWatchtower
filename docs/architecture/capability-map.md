...
| 层 | 位置 |
|---|---|
| 后端 | `app/recommendations/service.py`（准入引擎 + 粗排/融合）、`app/recommendations/{rerank,rubric,policy,reaggregate}.py`（L3 listwise 精排/导向编译/策略/反馈再估计）、`app/scoring/content_scorer.py` |
| API | `POST/GET /api/recommendation/runs`、`GET /api/recommendation/scorer-policy`、`POST /api/recommendation/scorer-preview`、`GET/PATCH /api/workspaces/{code}/recommendation-policy` + `compile-rubric`/`activate-rubric` |
| 前端 | `/recommendations`（分数拆解 + 评分策略摘要 + 单条评分预览 + P2/P3 观察池复核）、`/news` 与今日速览的准入徽章 |
| 数据 | `recommendation_runs/items`（admission_level/pool、noise_types、scorer_breakdown、expert_routes；WP4-A 增 coarse_score/llm_relevance_score/llm_rerank_status/llm_rerank_reason/rubric_hits_json/rubric_version）、`recommendation_rubric_compiles`、`source_score_snapshots`、`rubric_topic_priors`、`generation_daily_usage.purpose` 分桶 |
| 配置 | `config/scoring/content_scorer_v2.json`（阈值/权重/噪声规则/专家路由；`prompt_template` 已标 deprecated）、`config/scoring/rubrics/planning_intel_default.json`、`config/contracts/recommendation_ranking.json` |
...
`recommendation_reason`。AI 推荐三层管线已落地（2026-07-08，WP4-A，§4.1 行）：
规则粗排保留为行为基线（`llm_rerank_enabled=false` 默认排序与现状逐位一致的回归红线）、
L2 语义层接口预留默认关闭、L3 LLM listwise 分窗精排 + `final_score` 融合 +
内容导向 rubric 编译/版本化激活 + `purpose=rerank/rubric_compile` 预算分桶 +
每日 `feedback_reaggregate_daily` 源先验/主题权重再估计。
反馈回哺周/月工作流已实现（2026-07-08，WP4-G，§4.4）：`feedback_weekly_rollup`/
`feedback_monthly_review` 评估快照（precision@6/@12、rerank uplift、位次去偏
采信率等）与源分层/失效源 advisory 建议、rubric 修订提案人审链（accept 走既有
compile+activate 版本化链）、选择层 ε 探索位（默认 0 关闭 = 回归红线）与
`purpose=feedback_rollup` 预算桶（固定 4 次/工作台/日），契约
`recommendation_ranking.json` `feedback_workflow` 已为实现事实。
...
| LLM 生成 provider 分层配置 + 连通性自检 | D/H | `GENERATION_*` env 族与 `MINIMAX_*` 逐字段兼容回退；三条启动 fail-fast 规则落 `backend/app/core/deploy_checks.py`（契约已迁入 `startup_failfast_rules`）；`generation_policy` 读写（resolved provider/model/key_configured 永不回显 key、secret-like 422、审计 `workspace.generation_policy.update`）；`POST /api/generation/ping` 分类报错；`daily_generation_budget` 与 `fallback_behavior=fail` 语义；证据：`backend/tests/test_generation_provider.py`、`frontend/src/pages/WorkspaceSettingsPage.spec.ts`（生成模型卡）、`config/contracts/workspace_model.json` `generation_policy`、`config/contracts/deployment_modes.json` `startup_failfast_rules`。注：2026-07-08 决策变更 D-2026-07-08-KEY 的 provider 预设目录 + 密钥加密落库已随 WP4-B 落地（见下方专行），env 链保持兼容 |
| 模板驱动生成 generation_template（JSON/XML 载体，后端全链路，D-2026-07-08-TPL 语义） | D/E | `report_formats.generation_template(+_source)` 列与迁移；JSON/XML 无 DTD/外部实体解析到规范形；逐条新闻 × 逐启用模板格式各一次 LLM 格式化（模板字段全 AI 填充、outputSchema 全字段 + map_from 值作 `reference` 提示上下文与降级兜底）；整桶写 `generated_news.template_extras_json[format_code]`；投影只排版（rendition/MD/HTML 按模板字段序 label 渲染，零模型调用）；`template_fallback` 降级与 `regenerate` 只补缺失/过期桶；预算计入 `purpose=generation` 桶（公式 N×(1+F_daily)+W×F_weekly）；weekly 同机制（周报草稿构建/regenerate 位点，日报/周报格式双桶不互覆）；`POST /api/report-formats/validate-template` 干跑校验+示例预览（降级可兜底/纯生成字段划分 + 成本提示）；公司 SQL 任意模板配置下逐字节不变（负向断言 + `scripts/validate_company_sql.py` 用例）；证据：`backend/tests/test_generation_template.py`（§10.7 修订断言 1-10）、`config/contracts/report_renditions.json` `generation_template`。前端格式管理面板的模板上传/校验/预览 UI 未落地（见 §4.3 遗留）。2026-07-08 WP4-C 已按决策变更 D-2026-07-08-TPL 完成投影优先实现的重对齐 |
...
| AI 推荐三层管线（规则粗排基线 + 语义层接口预留 + LLM listwise 精排）、内容导向 rubric 编译/激活版本化、`final_score` 融合、`generation_daily_usage.purpose` 预算分桶、每日反馈再估计 | C | `recommendation_items` 新解释列 + `recommendation_rubric_compiles`/`source_score_snapshots`/`rubric_topic_priors` 表（迁移 `f1a2b3c4d5e6`）；`recommendation_policy` 读写 + `compile-rubric`（fingerprint 幂等预览、20 次/日固定桶）+ `activate-rubric`（7 天 fingerprint 门禁、`rubric_version+1`、审计）；L3 确定性分窗洗牌/锚点校准/失败重试/7 天结果缓存/漂移监控，全部降级路径不阻塞 run；`feedback_reaggregate_daily` 02:00 全量再估计（delta 硬界 ±6、主题权重 0.5w..1.5w）；`llm_rerank_enabled=false` 默认排序与现状逐位一致（回归红线断言）；证据：`backend/tests/test_recommendation_rerank.py`、`backend/tests/test_recommendation_policy.py`、`config/contracts/recommendation_ranking.json`、`config/scoring/rubrics/planning_intel_default.json` |
| Provider 预设目录（9 家 + custom 兜底）+ 密钥 Fernet 加密落库 + 生成模型卡完整配置流（决策变更 D-2026-07-08-KEY） | D/H | `llm_provider_credentials` 表（迁移 `f2b3c4d5e6a7`，排除 sync/导出）；`app/core/crypto.py` HKDF-SHA256 派生 + MultiFernet 轮换重加密；`GET /api/generation/providers` 目录投影 + credentials CRUD（super_admin，masked `****last4` 回显，审计无明文）；`resolve_generation_config` 凭据层（credential → env，`credential_missing` 不回落 env）；`generation_policy.credential_id` 指针 + ping 支持 `credential_id`；「enabled 且 env 无 key」从拒启降级 WARNING（`deployment_modes.json` `startup_warning_rules` 与 `deploy_checks.py` 1:1）；`/workspace-settings` 生成模型卡 provider 下拉/base_url 预填/key 写入/模型下拉/凭据选择/测试连通；证据：`backend/tests/test_credentials_api.py`、`backend/tests/test_generation_provider.py`、`frontend/src/pages/WorkspaceSettingsPage.spec.ts`、`config/contracts/llm_providers.json` |
| 报告页 ReportTimeline 时间轴 + 顶部筛选条 + 详情区 spacing 修正 + 文案违例 #1-#7 清理 | H | `ReportTimeline.vue` 按月分组/组头吸顶/状态点+条数徽章/无限滚动/快速跳月/失败重试行/空态；日报/周报页迁 `list*` + 时间轴侧栏 IA（已发布层 `GET /api/report-archive`、草稿层现有 reports API、viewer 不渲染草稿节点）；顶部标签胶囊/板块/关键词纯前端过滤；详情区按产品设计 §13.3 token 表逐元素修正（删 `.editorial padding-right: 0`）；证据：`frontend/src/components/ReportTimeline.spec.ts`、`frontend/src/pages/DailyReportsPage.spec.ts`、`frontend/src/pages/WeeklyReportsPage.spec.ts`、`backend/tests/test_report_archive_api.py`、`config/contracts/frontend_control_governance.json` `copy_audit_rule` |
| 排序一致性 + 空指标隐藏 + 界面文案审计看护 | C/H/Contract Test | Dashboard 头条候选改契约口径（`day_key=今日` 且 P0/P1/P2 的 top6 按 `final_score` 降序，非今日不入集合，无今日候选渲染空态）；`GET /api/dedupe-groups` 默认 `sort=score_desc`；日报 draft `sort_order` 按 `final_score` 降序；`rating_count=0` 隐藏平均评分、`final_score` 缺失显示「未评分」、归档页空样本隐藏均值；`test_blueprint_user_copy_bans_implementation_terms` 文案审计（banned_terms/patterns + exemptions，`known_violations` 已清零收紧）；证据：`frontend/src/pages/DashboardPage.spec.ts`、`NewsPage.spec.ts`、`DailyReportsPage.spec.ts`、`RecommendationsPage.spec.ts`、`backend/tests/test_news_api.py`、`backend/tests/test_blueprint_page_audit.py`、`config/contracts/recommendation_ranking.json` `ordering_consistency` |
| `/historical-reports` 跨来源归档重定位 | G/H | 页头改跨来源定位文案；已发布条目深链 `/daily-reports?report_id=...` / `/weekly-reports?report_id=...`（页内不渲染当前报告正文，legacy 保留页内阅读）；月份导航收敛为 legacy 视图筛选；summary 卡 current vs legacy 对比（空样本隐藏均值）；`GET /api/report-archive/summary` 支持 `report_type`/`origin` 过滤；证据：`frontend/src/pages/HistoricalReportsPage.spec.ts`、`backend/tests/test_report_archive_api.py`、`config/contracts/archive_knowledge.json` |
...
### 4.4 2026-07-08 第四轮实施收口

2026-07-08 第四轮设计定稿（四轨设计 + 交叉评审，实施拆包
`docs/implementation/01-implementation-plan.md` §18）的 WP4-A/B/C/D/E/F/G 已全部
实现，对应能力行已移入 §4.1（WP4-C 逐条模板格式化重对齐并入既有
generation_template 行；WP4-G 反馈回哺工作流并入 C 块推荐能力行）；契约状态位
（`recommendation_ranking.json` 主体与 `feedback_workflow`、
`llm_providers.json`、`workspace_model.json` `generation_policy.credential_id`、
`report_renditions.json` `generation_template`、`frontend_control_governance.json`
`copy_audit_rule`、`deployment_modes.json` `startup_warning_rules`）已同步为
实现事实。

WP4-G 反馈回哺工作流已于同日收尾实现（事实源
`docs/backend/feedback-heat-scoring.md` §11-§18，契约
`config/contracts/recommendation_ranking.json` `feedback_workflow`）：
`feedback_rollups`/`rubric_revision_proposals` 两表（迁移 `f3c4d5e6f7a8`，无
SyncMixin 不进 feed）、周/月 job、提案人审五端点、ε 探索位与
`purpose=feedback_rollup` 预算桶、前端「反馈回哺」区 +「反馈评估」卡；看护
`backend/tests/test_feedback_rollup.py` / `test_rubric_proposals.py` 与两页 spec。

本轮遗留（未实现，如实保留）：

- L2 语义层仍为接口预留：`news_item_embeddings` 表/模型已随迁移 `f1a2b3c4d5e6`
  留位但默认不写入，`config.py` 未引入 EMBEDDING_* env 族（env 样例无需新增），
  默认零调用零表写。
- 前端格式管理面板的模板上传/校验/预览 UI、`/ingestion-runs` 调度卡升级两项
  §4.3 遗留继续保留。
