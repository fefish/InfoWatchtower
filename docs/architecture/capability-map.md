...
### 4.3 设计已定稿待实现（2026-07-07 设计轮，未写码）

顶层设计已按实现级规格定稿并同步契约，等待实现；实现时以下列文档/契约为唯一验收基准，
不得实现成"看起来可用"的假控件：

| 能力 | 所属块 | 设计事实源 | 契约 | 判定标准 |
|---|---|---|---|---|
| 工作台级调度策略 + run 级自动重试 + 调度心跳可观测 | H/E | `docs/backend/pipeline-jobs-design.md` §6.1-§6.2、§8 | `config/contracts/workspace_model.json` `schedule_policy` | schedule-policy API 读写+审计、per-workspace 触发、失败 run backoff 重试链可追溯、`GET /api/pipeline/scheduler/status` 心跳/下次运行可自证；§12 断言 1-10 全绿 |
| LLM 生成 provider 分层配置 + 连通性自检 | D/H | `docs/backend/generation-provider-design.md` | `config/contracts/workspace_model.json` `generation_policy`、`config/contracts/deployment_modes.json` `planned_startup_failfast_rules`/`related_env` | key 只在实例 env（不回显）、工作台 generation_policy 读写+审计、`POST /api/generation/ping` 分类报错、`MINIMAX_*` 兼容回归；该文档 §7 断言 1-8 全绿 |
| 模板驱动生成（generation_template，JSON/XML 载体） | D/E | `docs/backend/reports-editorial-design.md` §8.1、`docs/backend/report-renditions-design.md` §10 | `config/contracts/report_renditions.json` `generation_template` | 投影优先/增量字段追加生成判定、模板校验+示例预览、template_fallback 降级、company SQL 逐字节不变；renditions 文档 §10.7 断言 1-10 全绿 |
| 布局模板与间距系统 + Dashboard 信息架构重排 | H | `docs/product/frontend-product-design.md` §9、`docs/product/page-specs/frontend-page-specs.md` §1.1/§3/§4.1 | —（前端体验强约束，无后端契约） | spacing tokens 全站唯一、四模板逐页标注、Dashboard 主列/固定侧栏归属与源健康折叠组件测试；产品设计 §9.5 断言全绿 |
| 统一弹窗系统（居中 Modal + 受限上下文面板） | H | `docs/product/frontend-product-design.md` §10、`docs/product/page-specs/frontend-page-specs.md` §3.1 | `config/contracts/frontend_control_governance.json` `modal_rule` | 4 处弹层迁居中 Modal（dialog/aria/焦点/脏表单确认）、config-panel 只保留白名单 2 处、validate_frontend_controls.py 扩展扫描；产品设计 §10.4 断言全绿 |
| 账号资料自助编辑（PATCH /api/auth/me） | H | `docs/backend/identity-access-design.md` §4.4、产品设计 §11 | `config/contracts/auth_modes.json` `profile_self_service` | 本地账号三字段可改+审计快照、外部身份 400、游客 403、胶囊即时刷新；identity 文档 §4.4 验收全绿 |
| 发现搜索 + 工作台加入码 + 公开形态矩阵 | H | `docs/backend/workspace-configuration-design.md` §14、产品设计 §12 | `config/contracts/workspace_model.json` `discovery_and_subscription`/`join_code`、`config/contracts/auth_modes.json` `identity_audit_actions` | discover?q 不泄露 private、join-code 三端点+join-by-code 幂等不降级、统一失效 400、限流 429、四类审计动作；workspace-config 文档 §14.4 验收全绿 |

配套 UI 落位（实现时同步 `docs/product/page-specs/frontend-page-specs.md`）：
工作台配置中心「自动化」「生成模型」「可见性与加入码」三张卡、今日速览侧栏
调度心跳卡（§4.1 第 6 位）与抓取页调度卡、格式管理面板模板上传/校验/预览、
发现工作台 Modal（搜索 + 凭码加入）、账号页资料卡。
