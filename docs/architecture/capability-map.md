...
状态：2026-07-07 按部署拓扑收口（信任边界/同步游标语义/多工作台策略/全量 adapter/部署工件/测试看护）刷新。
四形态 × 能力 × 必跑测试矩阵见 `docs/backend/backend-capability-test-matrix.md`。
...
已支持 `api.semanticscholar.org/graph/v1/paper/search/bulk` 源、目标日期 `publicationDateOrYear` filter、optional `SEMANTIC_SCHOLAR_API_KEY` 和完整 paper payload 保留；11 类 source_type 已全部有真适配器
...
拉取器；实现状态表见 `docs/backend/backend-capability-test-matrix.md` §3），run 层
...
**Gap**：① `wx://` 公众号采集（C-1，契约已锁）：旧系统方案是外部 `wx-cli` 工具跑在
登录微信的 Windows 机器上按日期窗口拉取（`references/参考工具/app.py`
`wechat_cli_fetch_windows`），纯服务端无法复刻；落地路径已定稿为「wx 桥接 sidecar
（宿主机小 HTTP 服务，`WX_BRIDGE_URL/WX_BRIDGE_TOKEN`）+ `wechat` 真适配器」，契约见
`docs/deployment/deployment-topology.md` §5.1；外部前置是向同事确认 wx 二进制事实。
② 超出 RSS 窗口的深度历史补采（P1）
...
**Gap**：① 生产主库执行同一套全量导入验收（P0，一条命令级动作+机器验收+缺口人工复核）② 跨对象体验和更多协作对象解释关系深化（P2）。
...
Playwright e2e 脚手架（`frontend/playwright.config.ts` + `e2e/smoke.e2e.ts`）已交付。
