...
账户生命周期 WP1 已补齐：后端新增 `user_invites/password_reset_tokens/login_attempts`，支持邀请建号、撤销、改密、忘记密码恒定响应、管理员代重置、登录限流、改密后旧 cookie 失效、`AUTH_SESSION_SECRET` 启动自检和 OIDC adapter Protocol；前端新增 `/invite/:code`、`/account`，用户页支持邀请、重置和启停。主要工作台业务 API 已统一执行 membership：viewer 只读，member 可采信/编辑/发布/导出，admin 可管理标签策略、源、流水线 run 和成稿格式；无 `workspace_code` 的全局列表仍限 super_admin。

开箱部署 WP3 已补齐：`deploy/install.sh --local|--domain` 会生成本地/生产 env、随机数据库密码和 `AUTH_SESSION_SECRET`、启动 Compose 并轮询 `/healthz`；backend 容器入口先跑 `alembic upgrade head`，worker/scheduler/reverse_proxy 等 backend healthy；没有 bootstrap 管理员时 `/setup` 创建首个 super_admin 并可选导入种子源；`APP_ENV=production` 缺 `DATABASE_URL` 或 public password 缺 `AUTH_SESSION_SECRET` 会启动失败；`deploy/upgrade.sh` 提供 pull/build/restart 升级路径，备份/恢复脚本可按 `docs/deployment-ops.md` 演练。

全局端到端验收已脚本化：`scripts/run_full_acceptance.py` 会生成本地 RSS feed 并通过真实 API 完成 Setup/登录、邀请用户、自助建台、启用共享源+自建 RSS 源、自定义标签策略、注册自定义周报格式、日报流水线、日报/周报 MD/HTML 导出、公司 SQL 预览、`scripts/validate_company_sql.py` 校验和 PostgreSQL 备份证据。pytest 入口为 `backend/tests/test_full_acceptance_script.py`，证据目录默认写入 `outputs/acceptance/<timestamp>/`。

可扩展桌面 WP2 已补齐：侧边栏建台入口改为三步向导（基本信息、共享源/自建源、标签策略），`super_admin/editor_admin` 可自助创建工作台；`PATCH /api/workspaces/{code}` 支持超管改名、改描述、启停和默认 domain，且 `planning_intel` 不可停用；`GET/POST/DELETE /api/workspaces/{code}/members` 和 `/users` 工作台成员 tab 支持 owner/admin 管理成员，非超管工作台列表只返回所属 membership；`config/domain_packs/hardware.json` 作为第二板块样例注册 `hardware_categories`，扩展配方见 `docs/extension-recipes.md`。
...
同步包状态补充：`POST /api/sync/packages/import` 已从“只写 `sync_inbox`”升级为对 `data_sources/raw_items/news_items` 执行业务 apply；重复 `event_id` 仍幂等跳过，revision/hash 冲突写入 `sync_conflicts` 而不覆盖本地对象。
...
- 已完成：本地可运行 monorepo、登录/RBAC、账户邀请/改密/限流、共享数据源池、Tech Insight Loop 源治理导入、Tech Insight Loop 旧资产只读盘点、历史导入 dry-run 和真实导入脚本、历史反馈/旧任务归档脚本、工作台统一标签策略、adapter 框架、抓取入 raw、标准化入 news、去重组、结构化内容准入评分、推荐、MiniMax 结构化生成、日报发布、公司 SQL 导出、周报采信项管理、需求/任务/同步/审计 v1。
- 已可验收：`/sources` 管理共享源、Tech 源导入和工作台标签策略；`/ingestion-runs` 查看抓取/补采覆盖；`/news` 查看去重 winner、准入字段和推荐/日报追溯；`/daily-reports` 生成、编辑、发布日报；`/weekly-reports` 生成并管理周报候选；`/exports` 生成和下载 SQL；`scripts/run_full_acceptance.py` 跑通蓝图 §9 全链路烟测。
...
- 当前主要缺口：更多同步 object_type 与冲突解决 UI、导出前字段长度/URL 长度/HTML 污染摘要、超出 RSS 当前窗口的深度历史补采、Tech Insight Loop 生产库实机全量导入验收、周报自动正文、从日报/周报沉淀 insight/requirement/task 的完整追溯、生产服务器/生产库实机运行 `scripts/run_full_acceptance.py` 后的外部证据归档。本地干净 Docker 证据已归档到 `outputs/acceptance/docker_full_20260703045331/`；本地隔离 PostgreSQL Tech 全量导入证据已归档到 `outputs/tech_insight_loop/postgres_full_import_20260703T050653Z/`。
