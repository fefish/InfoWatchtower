...
当前进度：阶段 0-2 已完成；阶段 3 已完成旧种子源导入、共享数据源池、默认工作台源链接、工作台统一标签策略、adapter 框架和手动 RSS/paper RSS 抓取到 `raw_items` 的最小链路。前端首页必须显示阶段 3，数据源页标题必须是“数据源、标签配置与 RSS raw 入库”，并能配置工作台统一一级标签策略、通过“配置”修改单源启用/权重/日限/可选来源提示、通过“抓取”触发单源入库。下一步补抓取调度，然后进入阶段 4 的 raw 到 news 标准化与去重。
