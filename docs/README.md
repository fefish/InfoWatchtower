...
当前进度：阶段 0-2 已完成；阶段 3 已完成旧种子源导入、共享数据源池、默认工作台源链接、工作台统一标签策略、adapter 框架和手动 RSS/paper RSS 抓取到 `raw_items` 的最小链路。前端首页必须显示阶段 3；工作台壳和导航必须来自后端工作台配置；数据源页采用信息流式共享源列表，右侧展示工作台统一一级/二级标签策略，并通过“配置”修改单源启用、权重和日限，通过“抓取”触发单源入库。`planning_intel` 与 `ai_tools` 的默认标签策略必须保持后端隔离。下一步补抓取调度，然后进入阶段 4 的 raw 到 news 标准化与去重。
