...
- 工作台统一标签策略保存在 `workspaces.config_json.label_policy`。模型生成新闻结构和去重后标签定稿都必须读取这套策略；单个数据源的标签路径只是来源提示，不是合法标签列表本身。
...
当前进度：已实现旧种子源导入 API、数据源列表 API、工作台统一标签策略 API、工作台源链接配置 API 和单源手动抓取 API。导入后 113 个源进入共享数据源池，并为 `planning_intel`、`ai_tools` 等已启用默认工作台创建 `workspace_source_links`；每个工作台当前 79 个源启用、34 个源停用，继承旧源 enabled 状态。管理员可在数据源页配置当前工作台统一一级标签策略；该策略是模型生成新闻结构和去重后标签定稿的合法标签列表。单个源只配置启用、权重、日限和可选来源提示。RSS/paper RSS 源可手动触发抓取到 `raw_items`，重复抓取按 `(data_source_id, entry_key)` 幂等更新。
...
- 前端首页和数据源页必须显示当前阶段 3 进度；数据源页应能配置工作台统一一级标签策略，并能配置单源启用/权重/日限/可选来源提示、手动触发 RSS/paper RSS 抓取。
