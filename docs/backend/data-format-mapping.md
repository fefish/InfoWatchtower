...
当前 SQL 导出预检、导出任务、合规门禁、trace 和下载权限的目标态事实源是
`docs/backend/export-compliance-design.md`。本文保留为字段映射和公司 SQL 形态细节附录。
...
category 取值模式由工作台策略的 `export_category_mode` 决定，**当前契约只支持
`news_primary` 单一取值**（`config/contracts/label_model.json`
`export_category_mode_contract`）：标签策略写入 API 与导出器双向锁死，导出侧对
非 `news_primary` 显式抛错而不是静默回退。公司 SQL 出口本身也按工作台策略
gating：只有解析为 `news_format_code=company_sql_v1` 且一级标签属于 AI 十分类的
工作台可导出，其余工作台 preflight 报 `workspace_not_company_sql`、导出 API 400。
...
原始发布时间并按北京时间 `Asia/Shanghai` 渲染，和日报 `day_key` 归属口径保持一致；如果来源缺失发布时间，标准日报 SQL 兜底为 `daily_reports.day_key 09:00:00`，
