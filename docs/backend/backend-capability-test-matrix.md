...
> 11 类 source_type adapter 的实现状态。
...
| adapter / 抓取 run | `test_adapters.py`、`test_adapter_{wiseflow,crawler,csv,paper_page,push_based}.py`、`test_ingestion_runs.py`、`test_ingestion_fetch.py` | 各 adapter fetch 契约、raw 幂等刷新、`skipped_unimplemented` 语义、失败源重试 |
...
## 3. 11 类 source_type adapter 实现状态

原审计口径「6 个 stub adapter」已在 2026-07 本轮由真实现替换完毕：
`backend/app/adapters/stubs.py` 仅保留给 `skipped_unimplemented` 语义的回归测试
显式注册（模块 docstring 明确禁止进默认注册表）；
`create_default_registry`（`backend/app/adapters/__init__.py`）注册的 11 类全部为真适配器。
...
| internal | `push_based.py InternalSourceAdapter` | ✅ 推入式/可拉取 | 默认推入式；配置 `fetch_config.api_url` 后升级为通用 JSON API 拉取器（`auth_token_env` 待迁 credential_ref） |

计划中的第 12 类：`wechat`（wx 桥接 sidecar + adapter，C-1，外部前置是确认
wx 二进制事实，见 `docs/deployment/deployment-topology.md` §5.1）。

### 3.1 明确的后续任务：推入式/低产出源在 UI 与导入时的提示

六类新 adapter 落地后遗留一个**产品侧任务**（前端所有权，尚未实现）：

- `manual/internal`（未配 `api_url`）是推入式语义，定时抓取恒 0 条新增是**正常行为**；
  数据源页与抓取 run 结果页应对这两类源展示「推入式源：由手工导入/内部系统写入，
  定时抓取不产出」的说明徽标，而不是让 0 条被误读为源坏了。
- 导入种子（legacy/tech）时，预览与结果面板应按 source_type 分组给出同样的语义提示
  （尤其 31 个 `wx://` 待补入口 metadata-only 源仍不可抓取，需要明示等待 wechat adapter）。
- run 摘要中的 `skipped_unimplemented` 前端文案保留（未来新增未实现类型时的安全网）。

验收标准：`SourcesPage.spec.ts` / `IngestionRunsPage.spec.ts` 增加对应组件测试；
文案映射进 `config/contracts/frontend_control_governance.json` 扫描面。
...
当前 28 个 spec、194 个用例全绿（2026-07-07 实跑，`npx vitest run`）。
...
- 壳与基建（4）：`layouts/AppShell.spec.ts`（顶部搜索/通知/部署徽标）、
  `router/index.spec.ts`（路由守卫/section 驱动）、`stores/runtime.spec.ts`
  （能力降级默认值）、`api/http.spec.ts`（CSRF 自动附头、`BASE_URL=/watchtower/`
  子路径前缀拼接）。

部署形态相关的前端看护点：runtime gate 关闭采集时隐藏入口、`limit<1` 本地拦截、
`no_sources` 警示、导入必须先 preview、read-only 隐藏写按钮。

e2e 现状：Playwright 脚手架已交付（`frontend/playwright.config.ts` +
`frontend/e2e/smoke.e2e.ts`，用例统一 `*.e2e.ts` 命名与 Vitest 收集规则分离；
smoke 用 `page.route` 打桩 `/api/**`，不依赖真实后端）。**尚未覆盖**真实后端的
关键旅程（登录 → 导入预览 → 抓取 → 日报 → 同步 → 导出），列为 P1 后续任务
（`docs/architecture/capability-map.md` §4.2）。
