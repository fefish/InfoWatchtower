...
| 用户组/批量入台/任务指派 | `test_user_groups.py`、`test_operations_api.py` | 组 CRUD 权限门（super_admin/editor_admin，viewer 403）、按组批量入台幂等且不降级已有角色/停用账号跳过/owner 角色 422、指派人必须是工作台成员（403）、指派触发站内通知、`assignee=me`/`assigned_to_me` 我的任务过滤 |
| 工作台可见性/发现/自助订阅（visibility + discover/subscribe） | `test_workspace_subscription.py`（8 用例） | 种子 visibility 初值（planning_intel=internal_public）、discover 不泄露 private、订阅幂等且不降级已有角色、退订仅限自己的 viewer membership、visibility PATCH 权限门、游客隐式 viewer 只读视角 |
| 游客登录（AUTH_GUEST_ENABLED） | `test_auth.py`（guest 段 6 用例）、`test_workspace_subscription.py` | 默认关（403）、runtime meta 下发开关、共享只读 guest 账号幂等复用且无密码、写操作集中 403 提示注册（仅放行 logout）、关闭开关存量会话立即失效、standalone/cloud 之外 fail-fast 拒启 |
