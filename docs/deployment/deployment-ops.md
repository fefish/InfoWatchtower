...
可选游客入口：`AUTH_GUEST_ENABLED=true`（默认 false，仅 `DEPLOY_MODE=standalone/cloud`
可开，其余形态启动自检拒绝）开启后登录页出现「以游客身份浏览」，签发共享只读 guest 会话：
只能浏览 `visibility=internal_public` 工作台的已发布内容，一切写操作 403 提示注册后可用。
语义见 `docs/deployment/auth-unified-login.md` §5.4。
