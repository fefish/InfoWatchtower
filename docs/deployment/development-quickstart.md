...
# 可选：scheduler 重启后错过触发点的补跑窗口（秒，默认 3600；超窗跳过、手动补跑）
export SCHEDULER_MISSED_WINDOW_SECONDS=3600
...
`GET /api/pipeline/scheduler/status`（已实现，见
`docs/backend/pipeline-jobs-design.md` §8.5；`/dashboard` 侧栏心跳卡即读该接口，
`GET /api/ingestion/scheduler` 仍保留 env 快照视图）。工作台级触发时刻/重试/
...
#      GENERATION_RETRY_TIMES / GENERATION_RETRY_BACKOFF_SECONDS

# 旧 MINIMAX_* 变量在弃用过渡期内保留为逐字段兼容回退（两者都配时 GENERATION_* 优先）：
