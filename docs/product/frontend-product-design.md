...
| 页面 | 前端任务 | 后端模块 |
|---|---|---|
| `/dashboard` 今日速览 | 看今日漏斗、头条候选、最新日报/周报、源健康；调度心跳/下次运行运营卡为设计已定稿待实现（`docs/backend/pipeline-jobs-design.md` §8.5） | pipeline/reports/sources |
...
| `/workspace-settings` 工作台配置 | 工作台内配置中心：基本信息、导航分区启停、标签策略、报告策略（自动发布）、成员、报告格式、可见性与加入码（§12）、自动化（调度时刻/重试/周报节拍/下次运行预览）与生成模型（模型参数/预算/连通状态，key 只显示已配置|未配置）两卡为设计已定稿待实现（`docs/backend/pipeline-jobs-design.md` §8.4、`docs/backend/generation-provider-design.md` §5）（admin/owner 可见；viewer 反馈策略仍在 `/users` 策略视图） | workspaces |
...
- 迁移完成后，`config-panel` class 只出现在 §10.2 判定通过的上下文面板
  （单源配置、成稿格式管理）；`scripts/validate_frontend_controls.py` 按
  `modal_rule` 扩展扫描（实现期任务）。
