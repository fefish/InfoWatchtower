...
- 「发现工作台」入口（工作台切换器底部，`frontend/src/components/WorkspaceDiscovery.vue`）：
  抽屉列出 `visibility=internal_public` 的可订阅工作台，支持自助订阅（viewer）与退订；
...
| `/workspace-settings` 工作台配置 | 工作台内配置中心：基本信息、导航分区启停、标签策略、报告策略（自动发布）、成员、报告格式（admin/owner 可见；viewer 反馈策略仍在 `/users` 策略视图） | workspaces |
