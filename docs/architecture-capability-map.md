...
现状：✅ 工作台可自助扩展且 seed 不覆盖自建台；导航/分区/格式全部数据库驱动；账户生命周期、
workspace membership、三步建台向导、成员管理、开箱部署和 `hardware` domain pack 样例已落地。
**Gap**：① 长期覆盖趋势与异常告警（P2）② 半导体等更多 domain pack 和快报 PPT 插件（P2）。
...
| P0 | 生产登录安全（限流/密码治理）+ 备份恢复演练 | H/F | ✅ 已落地；`scripts/run_full_acceptance.py` 已覆盖 §9 脚本化验收，本地干净 Docker 证据已归档到 `outputs/acceptance/docker_full_20260703045331/`，生产服务器/生产库实机输出待归档 |
