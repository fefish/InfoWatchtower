...
- **没有**：首次运行 Setup 向导、工作台成员管理界面、完整外部 OIDC provider 配置。
...
- **没有**：一键安装脚本、启动自动迁移、生产 env 完整自检、备份/恢复脚本、
  首次运行 Setup 向导。`AUTH_MODE=public_password` 缺 `AUTH_SESSION_SECRET` 的启动自检已补；
  scheduler 生产参数未在 env 模板中给出推荐值。
