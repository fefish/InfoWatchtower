...
账户生命周期与主要工作台 API membership 边界 ✅ WP1 已落地；开箱部署脚本、自动迁移、
Setup、生产 env 自检和备份/恢复脚本 ✅ WP3 已落地；建台向导、成员管理和
`hardware` domain pack ✅ WP2 已落地；全局 §8 端到端验收脚本与 pytest 已落地，干净
Docker/生产库走查可复用同一脚本留存实机证据。
```
...
- WP1 已补：`user_invites/password_reset_tokens/login_attempts`，邀请注册、撤销、
  公开邀请查询、改密、忘记密码恒定响应、管理员代重置、用户启停、登录限流、会话
  `updated_at` nonce、`AUTH_SESSION_SECRET` 启动自检、OIDC adapter Protocol、
  `/invite/:code` 和 `/account` 前端页、用户页邀请/重置/启停操作。
- membership 权限执行已接入主要工作台业务 API：workspace sections/label policy、
  sources、ingestion、news、recommendation、daily/weekly reports、renditions、exports；
  viewer 只读，member 可采信/编辑/发布/导出，admin 可管理源、标签策略、格式和流水线 run。
  后续新增带 `workspace_code` 的业务 API 必须复用 `assert_workspace_member`。
- 全局 §8 验收已脚本化：`scripts/run_full_acceptance.py` 会驱动 Setup/登录、邀请、
  自助建台、共享源+自建 RSS 源、标签策略、自定义周报格式、日报流水线、日报/周报
  MD/HTML 导出、公司 SQL 预览和 SQL 校验；pytest 入口为
  `backend/tests/test_full_acceptance_script.py`。干净 Docker 或生产库实机走查只需保留脚本
  输出和 `outputs/acceptance/<timestamp>/` 证据目录。
...
- 已有：`POST /api/workspaces` 建台（自动配齐核心分区/标签策略/内置成稿格式/owner
  成员）；`PATCH /api/workspaces/{code}`；`GET/POST/DELETE /api/workspaces/{code}/members`；
  `POST /api/sources` 自建源入共享池；标签策略 API；成稿格式注册表；分组导航
  （`workspace_sections.config_json.group`）；前端三步建台向导和 `/users` 工作台成员 tab；
  `config/domain_packs/hardware.json`。
- 已有：全局 §8 脚本化验收入口 `scripts/run_full_acceptance.py` 和
  `backend/tests/test_full_acceptance_script.py`；实机部署时将输出目录归档即可作为命令证据。

部署现状（`deploy/`）：

- 已有：`docker-compose.local.yml`、`docker-compose.prod.yml`（pg/redis/api/worker/
  scheduler/caddy）、`env.production.example`、部署检查脚本、PostgreSQL 备份/恢复脚本
  （支持生产/本地 compose 切换与 `postgresql+psycopg://` URL 规范化）、
  `deploy/install.sh`、`deploy/upgrade.sh`、backend 自动迁移入口、健康检查依赖、
  `/setup` 首次运行向导和生产 env 自检。
- 已有：全局 §8 验收脚本可在部署后执行；备份步骤在 PostgreSQL `DATABASE_URL` 下会调用
  `scripts/backup_db.sh` 产出备份证据。
...
实现方式：路由入口统一调用 `assert_workspace_member(session, user, workspace_code,
min_role=...)`；`config/contracts/auth_modes.json` 的 `route_min_roles` 是机器可读边界表。
...
4. viewer 成员可读日报、news、source、format、export trace，但对
   `PATCH /api/daily-report-items/{id}`、发布、反馈和格式创建得 403；member 可编辑、
   反馈、发布和创建公司 SQL 导出；workspace admin 可自建源、改源配置和注册成稿格式；
   非成员对该工作台业务 API 得 403；super_admin 不受限。
...
在干净 Docker 环境执行一次完整走查并留下记录（截图或脚本输出）。优先使用脚本化入口：
...
如果脚本直接在后端可访问 RSS server 的同一网络命名空间运行，可省略 `--rss-bind-host` 和
`--rss-public-host`。脚本会把证据写入 `outputs/acceptance/<timestamp>/`。
